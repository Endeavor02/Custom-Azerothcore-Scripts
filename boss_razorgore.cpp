...
public:
    ...
    struct boss_razorgoreAI : public BossAI
    {
        ...
        std::list<GameObject*> EggList;
        GameObject* EggArray[30];
        boss_razorgoreAI(Creature* creature) : BossAI(creature, BOSS_RAZORGORE) { }
        void getEggData()
        {
            me->GetGameObjectListWithEntryInGrid(EggList,177807,5000.0);
            int k = 0;
            for (GameObject*& i : EggList)
            {
                EggArray[k++] = i;
            }
        }
